...
class SceneImageDataset(data.Dataset, Configurable):
    r'''Prepairing dataset for scene text detection in BKAI OCR track
    '''
    data_dir = State()
    data_list = State()
    processes = State(default=[])
    ...
    def get_all_samples(self):
        with open(self.data_list[0], 'r') as f:
            image_list = json.load(f)
        self.image_paths = [item['image_path'] for item in image_list]
        self.gt_paths = [item['gt_path'] for item in image_list]
