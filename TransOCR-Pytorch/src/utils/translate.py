...
from src.model.transformerocr import src
from src.model.vocab import Vocab
from src.model.beam import Beam

def batch_translate_beam_search(img, model, beam_size=8, candidates=1, max_seq_length=128, sos_token=1, eos_token=2):
    # img: NxCxHxW
    model.eval()
    device = img.device
    sents = []
...
def build_model(config):
    ...
    #model = torch.nn.DataParallel(model) # ------------------------Update Da
