...
def transform_point(points, M):
  new_points = []
  for i in range(4):
    x = points[i * 2]
    y = points[i * 2 + 1]

    new_points.append(max(0, int(x * M[0][0] + y * M[0][1] + M[0][2])))
    new_points.append(max(0, int(x * M[1][0] + y * M[1][1] + M[1][2])))

  return new_points


def crop_nonrectangle(image, bboxes):
  edge_x = bboxes[4] - bboxes[6]
  edge_y = bboxes[5] - bboxes[7]

  cos_angle = edge_x / math.sqrt(edge_x * edge_x + edge_y * edge_y)

  if edge_x * edge_y >= 0:
    M = cv2.getRotationMatrix2D((bboxes[6], bboxes[7]), math.acos(cos_angle) * 180 / math.pi, 1)
  else:
    M = cv2.getRotationMatrix2D((bboxes[6], bboxes[7]), -math.acos(cos_angle) * 180 / math.pi, 1)
  bboxes = transform_point(bboxes, M)
  rotated = cv2.warpAffine(image, M, (image.shape[1], image.shape[0]))
  rotated = cut_img(rotated, bboxes)
        ...
        for img_path in tqdm(train_img_paths):
            ...
            for offsets, text in get_info_from_img_name(img_path,config):
                try:
                    if "##" in text:
                        continue
                    crop_img = crop_nonrectangle(img, (int(offsets[0]),int(offsets[1]),int(offsets[2]),int(offsets[3]),int(offsets[4]),int(offsets[5]),int(offsets[6]),int(offsets[7])))
                    # crop_img.save(os.path.join(save_all_img_folder,f'{idx}.jpg'))
                    cv2.imwrite(os.path.join(save_all_img_folder, f'{idx}.jpg'), crop_img)
