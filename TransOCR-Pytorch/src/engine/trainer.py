...
from src.losses.cer import char_error_rate
...
class Trainer():
    ...
    def train(self):
        ...
        for i in range(self.num_iters):
            ...
            if self.valid_annotation and self.iter % self.valid_every == 0:
                train_loss = self.validate(mode='train')
                acc_full_seq, acc_per_char, cer = self.precision(self.metrics, mode='train')

                info = 'iter: {:06d} - train loss: {:.3f} - acc full seq: {:.4f} - acc per char: {:.4f} - CER: {:.4f}'.format(
                    self.iter, train_loss, acc_full_seq, acc_per_char, cer)
                print(info)
                self.logger.log(info)
                ...
                if (acc_per_char + acc_full_seq) / 2 > best_acc:
                    self.save_weights(self.export_weights + '_best.pth')
                    best_acc =  (acc_per_char + acc_full_seq) / 2
                self.save_weights(self.export_weights + '_last.pth')
    ...
    def validate(self, mode='val'):
        ...
        if mode == 'val':
            loader = self.valid_gen
        else:
            loader = self.train_gen
    ...
    def predict(self, sample=None, mode='val'):
        pred_sents = []
        actual_sents = []
        img_files = []
    ...
    def precision(self, sample=None, mode='val'):

        pred_sents, actual_sents, _, _ = self.predict(sample=sample, mode=mode)

        acc_full_seq = compute_accuracy(actual_sents, pred_sents, mode='full_sequence')
        acc_per_char = compute_accuracy(actual_sents, pred_sents, mode='per_char')
        cer = char_error_rate(pred_sents, actual_sents) 
